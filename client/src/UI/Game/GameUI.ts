class GameUI extends eui.Component {
    ...
    private drawDiscardSPs(){
        ...
        this._discardSPList0.filter((v)=>GameMode.jokerPi.indexOf(v[0]) == -1).forEach((value,key)=>{
            value.forEach((v,k)=>{
        ...
        this._discardSPList3.filter((v)=>GameMode.jokerPi.indexOf(v[0]) == -1).forEach((value,key)=>{
            let arr = value.find(v=>v==-1) ? [-1,-1,-1,-1] : value;
            arr.forEach((v,k)=>{
        ...
        this._discardSPList2.filter((v)=>GameMode.jokerPi.indexOf(v[0]) == -1).forEach((value,key)=>{
            let arr = value.find(v=>v==-1) ? [-1,-1,-1,-1] : value;
            arr.forEach((v,k)=>{            
        ...
        this._discardSPList1.filter((v)=>GameMode.jokerPi.indexOf(v[0]) == -1).forEach((value,key)=>{
            let arr = value.find(v=>v==-1) ? [-1,-1,-1,-1] : value;
            arr.forEach((v,k)=>{   
    ...
    private posSource1 = ['xi_zhongjian_png', 'bei_zhongjian_png', 'dong_zhongjian_png', 'nan_zhongjian_png'];
    private posSource2 = ['bei_zhongjian_png', 'dong_zhongjian_png', 'nan_zhongjian_png', 'xi_zhongjian_png'];
    private posSource3 = ['dong_zhongjian_png', 'nan_zhongjian_png', 'xi_zhongjian_png', 'bei_zhongjian_png'];
    private posSource4 = ['nan_zhongjian_png', 'xi_zhongjian_png', 'bei_zhongjian_png', 'dong_zhongjian_png'];

    private posbgSource1 = ['xi_zhongjian_select_png', 'bei_zhongjian_select_png', 'dong_zhongjian_select_png', 'nan_zhongjian_select_png'];
    private posbgSource2 = ['bei_zhongjian_select_png', 'dong_zhongjian_select_png', 'nan_zhongjian_select_png', 'xi_zhongjian_select_png'];
    private posbgSource3 = ['dong_zhongjian_select_png', 'nan_zhongjian_select_png', 'xi_zhongjian_select_png', 'bei_zhongjian_select_png'];
    private posbgSource4 = ['nan_zhongjian_select_png', 'xi_zhongjian_select_png', 'bei_zhongjian_select_png', 'dong_zhongjian_select_png'];
    private setZj(pos) {
    ...
    private chowContainer:eui.Component;

    private hideChowChoice() {
        if (this.contains(this.chowContainer)) {
            this.removeChild(this.chowContainer);
        }
    }

    private showChowChoice() {
        var des = 80;
        ...
        if (GameMode.canChowChoice.length > 0) {
            GameMode.canChowChoice.forEach((arr,arrkey)=>{
                arr.map((v,k) => {
                    var card = new CardUI(2, v);
                    this.chowContainer.addChild(card);
                    var scale = 0.7;
                    card.scaleX = scale;
                    card.scaleY = scale;
    ...
    private drawOtherCard(discardList){
    ...
    private _roomId:eui.Label;
    private _total:eui.Label;
